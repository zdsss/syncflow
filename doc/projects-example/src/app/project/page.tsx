...
function SwimlaneView({ data }: { data: SwimlaneData }) {
  return (
    <div className="p-4">
      ...
      <div className="flex mb-4">
        ...
        {data.stages.map((stage, i) => (
          <div key={i} className="flex-1 text-center">
            <div className={cn('py-2 px-3 rounded-md text-xs font-medium mx-1', stage.progress === 100 ? 'bg-blue-500 text-white' : stage.progress > 0 ? 'bg-blue-200 text-blue-800' : 'bg-gray-100 text-gray-500')}>
              {stage.name}
      ...
      {/* Review Nodes Row */}
      <div className="flex mb-4">
        <div className="w-20 shrink-0 text-[10px] text-gray-400 text-right pr-3">评审节点</div>
      ...
      <div className="space-y-1">
        {data.lanes.map((lane, laneIdx) => (
          <div key={laneIdx} className="flex items-stretch">
            <div className="w-20 shrink-0 text-xs text-gray-600 font-medium pr-2 flex items-center border-r border-gray-200">
              {lane.name}
            </div>
            <div className="flex-1 flex items-center gap-2 px-3 py-2 border-b border-gray-50 min-h-[36px]">
              {lane.tasks.map((task, taskIdx) => {
                ...
                return (
                  <div
                    key={taskIdx}
                    className={cn(
                      'flex items-center gap-1.5 px-2 py-1 rounded text-[11px] cursor-pointer hover:opacity-80',
                      isDone ? 'bg-blue-100 text-blue-700' : isActive ? 'bg-amber-100 text-amber-700' : 'bg-gray-50 text-gray-500'
                    )}
                  >
                    <span className={cn('w-2 h-2 rounded-full', isDone ? 'bg-blue-500' : isActive ? 'bg-amber-500' : 'bg-gray-300')} />
                    {task}
                  </div>
                );
              })}
